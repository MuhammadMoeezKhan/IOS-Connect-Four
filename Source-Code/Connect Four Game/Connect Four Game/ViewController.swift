...
class ViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource
{
    ...
    func collectionView(_ cv: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        ...
        if var boardItem = getLowestEmptyBoardItem(column)
        {
            if let cell = collectionView.cellForItem(at: boardItem.indexPath) as? BoardCell
            {
                ...
                //if victory has been detected using Board.swift
                //increment the score of the team (based on color) that won
                //If victory, also alert the updated score onto user's screen
                if victoryAchieved()
                {
                    if yellowTurn()
                    {
    ...
    //Set all the cells in the grid back to default color:
    //white
    func resetCells()
    {
